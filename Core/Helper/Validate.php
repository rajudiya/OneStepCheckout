...
class Validate extends AbstractData
{
    ...
    {
        if (is_null($this->_seofebModules)) {
            $this->_seofebModules = [];

            $moduleList = $this->_moduleList->getNames();
            foreach ($moduleList as $name) {
                if (strpos($name, 'Seofeb_') === false) {
                    continue;
                }

                $this->_seofebModules[] = $name;
            }
        }

        return $this->_seofebModules;
    }
}